...
class Solver:
    ...
    def run(self):
        ...
        for idx, _ in enumerate(self.input):
            if idx % 3 > 0:
                continue
            elf1 = set(self.input[idx])
            elf2 = set(self.input[idx+1])
            elf3 = set(self.input[idx+2])
            x = elf1.intersection(elf2).intersection(elf3)
            total += self.__letters.find(list(elf1.intersection(elf2).intersection(elf3))[0]) + 1
        return total
